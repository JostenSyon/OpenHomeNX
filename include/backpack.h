...
// Zaino Gen3 (RSE/FRLG): regala strumenti in borsa, traccia tutto in un
// giornale e valida il contenuto. Mai silenzioso, mai incoerente: id validi
// dal DB (romfs/data/items_gen3.json), count sempre <= max del pocket,
// tutto scritto via SaveFile (checksum ricalcolati da saveGBA).
namespace Backpack {

struct ItemDef {
    int id = 0;
    std::string name;
    std::string pocket; // "items"|"key"|"balls"|"tm"|"berries"
    bool key = false;
    int max = 99;
    bool ruby = false, emerald = false, frlg = false;
    bool flag = false; // biglietto evento: richiede anche il flag (WIP separato)
};
