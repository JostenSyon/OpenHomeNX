...
mod tests {
    ...
    fn all_forms_have_types() -> Result<(), String> {
        try_all_forms(|form| {
            types_lookup(form.national_dex, form.form_index, None)
                .ok_or(alloc::format!("Missing types for {}", form.form_name))?;
            Ok(())
        })
    }
    ...
    fn no_form_duplicates_type() -> Result<(), String> {
        try_all_forms(|form| {
            ...
            let (type1, type2) = types_lookup(form.national_dex, form.form_index, None)
                .ok_or(alloc::format!("Missing types for {form_name}"))?;
    ...
    fn no_zero_stats() -> Result<(), String> {
        try_all_forms(|form| {
            ...
            let stats = current_base_stats(form.national_dex, form.form_index)
                .ok_or(alloc::format!("Missing stats for {form_name}"))?;
            if stats.hp == 0
                || stats.atk == 0
                || stats.def == 0
                || stats.spa == 0
    ...
    #[test]
    fn pikachu_present_in_all_sources() -> Result<(), impl core::fmt::Debug> {
        METADATA_SOURCES_IMPLEMENTED
            .into_iter()
            .try_for_each(|source| {
                if !has_entry(source, NationalDex::Pikachu as u16, 0) {
                    Err(alloc::format!("Pikachu not found in metadata source {source:?}"))
                } else {
                    Ok(())
                }
            })
