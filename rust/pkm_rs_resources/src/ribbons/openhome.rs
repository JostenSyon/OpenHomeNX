...
impl IntoIterator for ObsoleteRibbonSet {
    ...
    type IntoIter = alloc::vec::IntoIter<ObsoleteRibbon>;
...
impl ObsoleteRibbon {
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        if !full_name.ends_with("Ribbon") && !full_name.ends_with("Mark") {
            full_name = alloc::format!("{name} Ribbon");
        }
...
impl<const MODERN_BYTE_COUNT: usize> OpenHomeRibbonSet<MODERN_BYTE_COUNT> {
    ...
    pub fn to_bytes(self) -> Vec<u8> {
        let mut bytes: Vec<u8> = alloc::vec![0; OBSOLETE_RIBBON_BYTES + MODERN_BYTE_COUNT];
