...
use pkm_rs_types::{AbilityNumber, FlagSet, Stats16Le};
...
fn flagset14_from_opt(v: Option<Vec<u8>>) -> FlagSet<14> {
    v.and_then(|b| <[u8; 14]>::try_from(b.as_slice()).ok())
        .map(FlagSet::from_bytes)
        .unwrap_or_default()
}

fn flagset8_from_opt(v: Option<Vec<u8>>) -> FlagSet<8> {
    v.and_then(|b| <[u8; 8]>::try_from(b.as_slice()).ok())
        .map(FlagSet::from_bytes)
        .unwrap_or_default()
}

impl OhpkmConvert for Pa8 {
    fn to_main_data(&self) -> ohpkm::v2_sections::MainDataV2 {
        ohpkm::v2_sections::MainDataV2 {
            ...
            ability_index: self
                .ability_index
                .change_bound()
                .expect("Pa8 max ability <= overall max ability"),
            ...
            moves: self
                .moves
                .to_pp_adjusted(MetadataSource::LegendsArceus, ohpkm::MOVE_METADATA_SOURCE),
    ...
    fn to_la_data(&self) -> Option<LegendsArceusData> {
        Some(LegendsArceusData {
            gvs: self.gvs,
            move_flags: FlagSet::from_bytes(self.move_flags_la.to_bytes()),
            tutor_flags: FlagSet::from_bytes(self.tutor_flags_la.to_bytes()),
            master_flags: FlagSet::from_bytes(self.master_flags_la.to_bytes()),
            is_alpha: self.is_alpha,
            is_noble: self.is_noble,
    ...
    fn from_ohpkm(ohpkm: &OhpkmV2, strategy: ConvertStrategy) -> Result<Self> {
        ...
        let converter = PkmConverter::new(PkmFormat::PA8, strategy);
        let met_data = converter.met_data(ohpkm);

        let ability_index = if let Some(current_ability) = ohpkm.ability_index().change_bound() {
            current_ability
        } else if let Some(ability_from_num) =
            form_metadata.get_ability(ohpkm.ability_num()).change_bound()
        {
            ability_from_num
        ...
        let mut mon = Self {
            ...
            is_noble: ohpkm.is_noble().unwrap_or(false),
            flag2_la: ohpkm.flag2_la().unwrap_or(false),
            ...
            scale: ohpkm.height_scalar(),
            contest_memory_count: ohpkm.contest_memory_count(),
            battle_memory_count: ohpkm.battle_memory_count(),
            alpha_move: ohpkm.alpha_move().unwrap_or(0),
            gvs: ohpkm.gvs().unwrap_or_default(),
            move_flags_la: flagset14_from_opt(ohpkm.move_flags_la()),
            tutor_flags_la: flagset8_from_opt(ohpkm.tutor_flags_la()),
            master_flags_la: ohpkm.master_flags_la().unwrap_or_default(),
            form_argument: ohpkm.form_argument(),
            affixed_ribbon: ohpkm.affixed_ribbon(),
            nickname: ohpkm.nickname(),
            moves: ohpkm
                .moves()
                .to_pp_adjusted(ohpkm::MOVE_METADATA_SOURCE, MetadataSource::LegendsArceus),
        ...
        bytes
            ...
            .map_err(|_| {
                Error::buffer_size_with_source(
                    "Pa8::OhpkmConvert::bytes_to_stored",
                    gen8_la::PKM_DATA_SIZE,
                    bytes.len(),
                )
            })
            .map(StoredPkmBytes::Pa8)
    }
}
