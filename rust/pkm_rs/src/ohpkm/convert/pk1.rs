...
use crate::gen1::{Pk1, BOX_SIZE, PARTY_SIZE};
...
impl OhpkmConvert for Pk1 {
    fn to_main_data(&self) -> ohpkm::v2_sections::MainDataV2 {
        let ndex = self.national_dex;
        ...
        ohpkm::v2_sections::MainDataV2 {
            ...
            trainer_friendship: 0,
            ball: Default::default(),
            met_location_index: 0,
            met_level: self.level,
            ...
            return extended
                ...
                .map_err(|_| {
                    Error::buffer_size_with_source(
                        "Pk1::OhpkmConvert::bytes_to_stored",
                        PARTY_SIZE,
                        extended_len,
                    )
                })
                .map(StoredPkmBytes::Pk1);
        }
        bytes
            .try_into()
            .map_err(|_| {
