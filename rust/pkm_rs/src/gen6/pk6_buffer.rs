...
use super::{Pk6AbilityIndex, Pk6SpeciesAndForm};
...
use pkm_rs_types::{
    AbilityNumber, BinaryGender, ContestStats, Gender, Geolocations, Ivs,
    MarkingsSixShapes, OriginGame, PokeDate, Pokerus, Stats8, TrainerMemory,
};
...
pub type Pk6BufferMut<'a> = Pk6Buffer<&'a mut [u8]>;

#[derive(Default, Clone, Copy)]
pub struct Pk6Buffer<S: AsRef<[u8]>>(S);

impl<'a> Pk6Buffer<&'a [u8]> {
...
impl<S: AsRef<[u8]> + AsMut<[u8]>> Pk6Buffer<S> {
...
impl<S: AsRef<[u8]>> Pk6Buffer<S> {
    ...
    pub fn form_index_raw(&self) -> u8 {
        pkm_rs_types::read_uint5_from_bits(self.get_u8(Offset::FormIndexFatefulEncounterGender), 3)
    }

    pub fn species_and_form(&self) -> Result<Pk6SpeciesAndForm> {
        let species_and_form = SpeciesForm::new(self.species_ndex(), self.form_index_raw().into())?;
        Pk6SpeciesAndForm::try_new(species_and_form).ok_or(Error::form_index(species_and_form))
    }

    // For Pikachu cosplay, the raw form index 1..6 maps to extra. This returns the
    // underlying species form with form 0 if cosplay detected, else same as species_and_form.
    // Caller must handle extra manually.
    ...
    pub fn training_bag_hits(&self) -> u8 {
        self.get_u8(Offset::TrainingBagHits)
    }

    pub fn training_bag(&self) -> u8 {
        self.get_u8(Offset::TrainingBag)
    }

    pub fn markings_raw(&self) -> u8 {
        self.get_u8(Offset::Markings)
    }

    pub fn markings(&self) -> MarkingsSixShapes {
        MarkingsSixShapes::from_byte(self.markings_raw())
    }
    ...
    pub fn ribbons_raw(&self) -> [u8; 6] {
        self.get_array(Offset::Ribbons)
    }

    pub fn ribbons(&self) -> ModernRibbonSet<6, { super::MAX_RIBBON_KALOS }> {
    ...
    pub fn encounter_type(&self) -> u8 {
        self.get_u8(Offset::EncounterType)
    }
    ...
    pub fn status_condition(&self) -> u8 {
        self.get_u8(Offset::StatusCondition)
    }

    pub fn current_hp(&self) -> u8 {
        self.get_u8(Offset::CurrentHp)
    }
...
impl<S: AsRef<[u8]> + AsMut<[u8]>> Pk6Buffer<S> {
    ...
    pub fn set_oras_form_index(&mut self, v: u8) {
        self.set_forme_index(v);
    }
    ...
    pub fn set_ability_index(&mut self, v: Pk6AbilityIndex) {
    ...
    pub fn set_training_bag_hits(&mut self, v: u8) {
        self.set_u8(Offset::TrainingBagHits, v);
    }

    pub fn set_training_bag(&mut self, v: u8) {
        self.set_u8(Offset::TrainingBag, v);
    }
    ...
    pub fn set_markings(&mut self, v: MarkingsSixShapes) {
    ...
    fn set_ribbons_raw(&mut self, v: &[u8; 6]) {
        self.set_array(Offset::Ribbons, v);
    }

    pub fn set_ribbons(&mut self, v: ModernRibbonSet<6, { super::MAX_RIBBON_KALOS }>) {
    ...
    pub fn set_met_level(&mut self, v: u8) {
        // clear low 7 bits then set
        let b = &mut self.bytes_mut()[Offset::MetLevelTrainerGender as usize];
        *b = (*b & 0x80) | (v & 0x7F);
    }
    ...
    pub fn set_encounter_type(&mut self, v: u8) {
        self.set_u8(Offset::EncounterType, v);
    }
    ...
    pub fn set_status_condition(&mut self, v: u8) {
        self.set_u8(Offset::StatusCondition, v);
    }

    pub fn set_current_hp(&mut self, v: u8) {
        self.set_u8(Offset::CurrentHp, v);
    }
...
impl<S: AsRef<[u8]>> AsBytes for Pk6Buffer<S> {
    fn as_bytes(&self) -> &[u8] {
        self.0.as_ref()
    }
}

impl<S: AsRef<[u8]> + AsMut<[u8]>> AsBytesMut for Pk6Buffer<S> {
    fn as_bytes_mut(&mut self) -> &mut [u8] {
        self.0.as_mut()
    }
}

impl<S: AsRef<[u8]>> Checksum for Pk6Buffer<S> {
    type A = ChecksumU16Le;
    const SPAN_START: usize = 8;
    const SPAN_END: usize = super::BOX_SIZE;
}

impl<S: AsRef<[u8]> + AsMut<[u8]>> RefreshChecksum for Pk6Buffer<S> {
