...
// ---------------------------------------------------------------------------
// Pb8Buffer<S> — generic over the byte storage so that a single impl block
// covers all getters, and a second (narrower) block covers setters.
//
//   Pb8BufferRef<'a>  = Pb8Buffer<&'a [u8]>       — read-only
//   Pb8BufferMut<'a>  = Pb8Buffer<&'a mut [u8]>   — read + write
// ---------------------------------------------------------------------------

pub type Pb8BufferMut<'a> = Pb8Buffer<&'a mut [u8]>;

#[derive(Default, Clone, Copy)]
pub struct Pb8Buffer<S: AsRef<[u8]>>(S);

// ------------------------------------------------------------------
// Constructors — immutable
// ------------------------------------------------------------------

impl<'a> Pb8Buffer<&'a [u8]> {
...
impl<S: AsRef<[u8]> + AsMut<[u8]>> Pb8Buffer<S> {
...
impl<S: AsRef<[u8]>> Pb8Buffer<S> {
    ...
    pub fn species_and_form(&self) -> Result<Pb8SpeciesAndForm> {
        let species_and_form = SpeciesForm::new(self.species_ndex(), self.form_num())?;
        Pb8SpeciesAndForm::try_new(species_and_form).ok_or(Error::form_index(species_and_form))
    }
...
// ==================================================================
// Setters  (available only on Pb8BufferMut)
// ==================================================================
...
impl<S: AsRef<[u8]> + AsMut<[u8]>> Pb8Buffer<S> {
    ...
    pub fn set_ability_index(&mut self, v: Pb8AbilityIndex) {
...
impl<S: AsRef<[u8]>> AsBytes for Pb8Buffer<S> {
    fn as_bytes(&self) -> &[u8] {
        self.0.as_ref()
    }
}

impl<S: AsRef<[u8]> + AsMut<[u8]>> AsBytesMut for Pb8Buffer<S> {
...
impl<S: AsRef<[u8]>> Checksum for Pb8Buffer<S> {
    type A = ChecksumU16Le;
    const SPAN_START: usize = CHECKSUM_START;
    const SPAN_END: usize = CHECKSUM_END;
}

impl<S: AsRef<[u8]> + AsMut<[u8]>> RefreshChecksum for Pb8Buffer<S> {
