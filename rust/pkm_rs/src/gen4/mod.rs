...
// PKHeX Diamond/Pearl level-up learnset table, indexed by national dex.
static DP_LEARNSETS: LearnsetFileReader = LearnsetFileReader::from_pkl_bytes(
    include_bytes!("../../../pkm_rs_resources/src/pkhex_bin/levelup/lvlmove_dp.pkl"),
);

/// True if this species exists in Gen 4 (Diamond/Pearl metadata): the
/// dex-cut gate for downgrades. Same data PKHeX/HOME enforce.
pub fn species_legal_in_gen4(ndex: u16) -> bool {
    pkm_rs_resources::species::form_metadata::source_has_form_metadata(
        pkm_rs_resources::metadata_source::MetadataSource::DiamondPearl,
        ndex,
        0,
    )
}
...
/// True if this move id may appear on a Gen 4 record: empty slot or a move
/// introduced in Gen 4 or earlier. Newer moves are dropped explicitly by
/// our FFI layer instead of leaking through.
pub fn move_legal_in_gen4(move_id: u16) -> bool {
    if move_id == 0 {
        return true;
    }
    MoveIndex::from_u16(move_id)
        .get_metadata()
        .is_some_and(|md| md.introduced <= Generation::G4)
}

/// Base moves for a Gen 4 record of this species: the head of its
/// Diamond/Pearl learnset (up to 4), each with its base PP. Pound (id 1)
...
pub fn base_moves_for_species(ndex: u16) -> ([u16; 4], [u8; 4]) {
    ...
    let mut filled = 0;
    if ndex != 0 {
        if let Some(reader) = DP_LEARNSETS.learnset_at_index(ndex) {
            for m in reader.all_moves() {
                if filled >= 4 {
                    break;
                }
                let id = m.move_id_raw();
                if id == 0 || !move_legal_in_gen4(id) {
                    continue;
                }
                moves[filled] = id;
