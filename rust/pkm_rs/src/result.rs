...
pub enum Error {
    ...
    FieldError {
        field: &'static str,
        source: Rc<dyn core::error::Error>,
    },
    TagError {
        tag_type: &'static str,
        value: u16,
...
impl Display for Error {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let message = match self {
            ...
            Self::BoxIndex(index) => alloc::format!("Invalid box index: {index}"),
            Self::BoxSlot(slot) => alloc::format!("Invalid box slot: {slot}"),
            Self::BufferSize {
                requirement_source,
                expected,
                received,
            } => match requirement_source {
                Some(source) => alloc::format!(
            ...
            Self::BuildSave { context, source } => match source {
                Some(source) => alloc::format!("Error opening save: {context}; original error: {source}"),
                None => alloc::format!("Error opening save: {context}"),
            },
            Self::NationalDex {
                value: national_dex,
                source,
            ...
            Self::PokemonGameIndex {
                ...
                alloc::format!("Invalid {source} index number {national_dex}")
            }
            Self::FormIndex {
                national_dex,
                form_index,
            ...
            Self::AbilityNumber(InvalidAbilityNumber(num)) => {
                alloc::format!("Invalid ability number {num} (must be between 1 and 3)")
            }
            Self::FieldError { field, source } => {
                alloc::format!("Self reading field {field}: {source}")
            }
            Self::TagError { tag_type, value } => {
                alloc::format!("Invalid tag value {value} for tag type {tag_type}")
            }
            Self::StringDecode { source } => alloc::format!("String decode error: {source}"),
